...
void  compute_gamma_D8(struct flow_struct *flow_table, int num_patches, int sc_flag, double cell,int algorithm_flag, int*pch_boun,int thread_inx)
{
	...
	//cout << "D8\nNumber of grids with specific number of flow routings:"<<thread_inx << endl;
	//for (int i = 0; i < 9; i++)
...
int CreatFlowTable::sort_flow_table(int *patch_pch, double *dem, int *patch, int maxr, int maxc)
{
	...
	cout << "sort grids in terms of elevation:";
	...
	CReadCommandline *commandline, double cell, char *input_prefix, double width, int maxr, int maxc, int algorithm_flag)
{
	...
	cout << "print .pch file that stores the order of grids in flow table:";
	...
	if (commandline->pch == true) {
		...
		PCH_dat << "ncols\t" << maxc<<"\n";
		PCH_dat << "nrows\t" <<  maxr << "\n";
		PCH_dat << "xllcorner \t" << 100000 <<  "\n";
		PCH_dat << "yllcorner\t" << 100000 <<  "\n";
		PCH_dat << "cellsize \t" << cell <<  "\n";
		PCH_dat << "NODATA_value\t" << -9999 <<  "\n";
...
{
	cout << "print _flow_table_D8.dat file that stores flow directions:";
